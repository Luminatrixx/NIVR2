...
Perceptually-accurate color correction methods to match upscaled video
frames to their original color characteristics. All methods preserve spatial
details while transferring color distributions.
...
Ported from seedvr2_videoupscaler's src/utils/color_fix.py.
"""
...
def safe_pad_operation(
    ...
    """
    Safe padding with automatic float16 compatibility handling.
...
def safe_interpolate_operation(
    ...
    """
    Safe interpolation with automatic float16 compatibility handling.
...
def ensure_float32_precision(
    ...
    (color space conversions, statistics, histogram matching, guided filtering).

    Returns (converted_tensor, original_dtype) so callers can restore the
    original dtype via `.to(original_dtype)` after computation.
    """
...
def hsv_saturation_histogram_match(content_feat: Tensor, style_feat: Tensor) -> Tensor:
    """
...
def wavelet_adaptive_color_correction(content_feat: Tensor, style_feat: Tensor) -> Tensor:
    """
    ...
    hsv_result = hsv_saturation_histogram_match(content_feat, style_feat)
